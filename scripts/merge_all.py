#!/usr/bin/env python3
"""Merge all changes: intro + derivation + ADE deepen + refs -> one final DOCX."""
...
intro = {
    ...
    15: (
        ...
        "polymerization. We systematically characterize ESPTs microstructure, "
...
for idx, txt in intro.items():
    p = doc.paragraphs[idx]
    for r in p.runs: r.text = ""
    p.runs[0].text = txt
print("1/4 Intro rewritten")

# ── 2. DERIVATION (para 126) ──
p126 = doc.paragraphs[126]
new126 = (
    "The interpretation model is derived from the one-dimensional "
    "advection-dispersion equation (ADE): partialC/partialt + v times "
    "partialC/partialx = D times partial2C/partialx2, where v = 4Q/(pi d2) "
    "is the mean flow velocity (cm/min) and D = alpha v is the longitudinal "
    "dispersion coefficient (cm2/min), with Q the effective oil-phase volumetric "
    "flow rate (cm3/min), d the tubing inner diameter (cm), and alpha the "
    "longitudinal dispersivity (cm). For a semi-infinite domain with an "
    "instantaneous slug of tracer mass M injected at x = 0, t = 0, the classical "
    "ADE analytical solution yields a Gaussian pulse: "
    "C_rise(x,t) = (M/A)/sqrt(4pi D t) exp[-(x - v t)2 / (4 D t)], "
    "which describes the advective-dispersive transport of the tracer "
    "accumulated in the proppant pack during the well shut-in period. "
    "For a continuous source maintaining a constant concentration C0 at the "
    "inlet, the ADE solution takes the complementary-error-function form: "
    "C_fall(x,t) = (C0/2) erfc[(x - v t) / sqrt(4 D t)], "
    "which captures the sustained release of residual nano-Fe3O4@SA from the "
    "epoxy matrix after the main concentration pulse has passed. These two "
    "transport regimes—the shut-in slug and the matrix-diffusion-controlled "
    "tail—operate simultaneously during production. To construct a physically "
    "continuous composite model that avoids the non-differentiable discontinuity "
    "of a piecewise step transition, we blend the two analytical solutions via a "
    "hyperbolic-tangent weighting function w(t) = 1/2 [1 + tanh((t - t0)/sigma)], "
    "where t0 is the crossover center and sigma controls the transition width. "
    "The resulting piecewise advection-dispersion model—incorporating both the "
    "Gaussian pulse and erfc tail components with smooth tanh transition—is "
    "expressed as Eq. (2)."
)
for r in p126.runs: r.text = ""
p126.runs[0].text = new126
print("2/4 Derivation added")
...
deep = (
    ...
    "controlled release mechanism identified in Section 3.6. Second, the tanh "
    "blending function provides a physically motivated, continuously "
    "differentiable transition between the Gaussian rise and erfc tail regimes. "
    "A piecewise step-function transition would introduce a non-physical "
    "discontinuity in the first derivative of the concentration profile at the "
    "crossover. Third, the close agreement between the pump-set flow rate "
    ...
    "physical meaning and can be recovered from the tracer signal alone. "
    "Fourth, the 47% erfc tail contribution quantitatively confirms that nearly "
    "half of the total detectable tracer signal originates from sustained "
    "matrix-diffusion-controlled release rather than from the initial "
    "concentration pulse. This establishes that ESP-T-based production "
...
if ref_start:
    ...
    for i in range(ref_start + 1 + len(refs), len(doc.paragraphs)):
        t = doc.paragraphs[i].text.strip()
        if t and (t[0] == "[" or "Reagents" in t or "Water passage" in t or "K-P model" in t):
            for r in doc.paragraphs[i].runs:
                r.text = ""
    print("4/4 References rebuilt")
else:
    print("WARNING: References heading not found!")

# ── SAVE ──
out = "四氧化三铁环氧树脂拟合/ESP-T_完整版.docx"
doc.save(out)
print(f"Saved: {out}")