# -*- coding: utf-8 -*-
"""
Definitive figure set. Max quality matplotlib settings.
SVG for journal submission, PDF for print, PNG for preview.
"""
import numpy as np
...
# ── Parameters ────────────────────────────────────────
cb=0.18003; A_amp=0.1000; a_tail=50.0; alpha=1200.0; Q_mL=0.4689; t0=16.338; s0=7.011
...
print(f'Q={Q_mL:.4f} dev={qdev:.1f}% Pe={Pe:.3f} MRT={MRT:.2f} R2={r2:.4f} G={gp:.0f}% E={ep:.0f}%')
...
def stat_box(ax,text):
    ax.annotate(text,xy=(0.97,0.97),xycoords='axes fraction',ha='right',va='top',
...
panel_label(ax,'(a) Model fit'); legend_box(ax)
ax.set_xlim(-2,112); ax.set_ylim(-0.05,1.15)
stat_box(ax,f'R$^2$={r2:.4f}\nGauss: {gp:.0f}%\nerfc:  {ep:.0f}%')
...
panel_label(ax,'(b) Residuals'); ax.set_xlim(-2,112)

ax=axes[2]
ax.barh(['Pump set','Fitted'],[0.50,Q_mL],height=0.4,color=[GRAY,RED],edgecolor='white')
ax.set_xlabel('Flow rate Q (mL/min)')
panel_label(ax,'(c) Flow rate validation'); ax.set_xlim(0,0.6)
for v,y in[(0.50,0),(Q_mL,1)]: ax.text(v+0.01,y,f'{v:.2f}',va='center',fontsize=10,fontweight='bold')
ax.annotate(f'{qdev:.0f}% difference',xy=(0.48,0.3),fontsize=9,color=RED,fontweight='bold')
...
rK=[0.95489,0.9649,0.95599,0.94537]; kc=[BLUE,ORANGE,RED,PURPLE]; mk=['o','s','^','D']
...
panel_label(ax,'(a) Release profiles'); legend_box(ax); ax.set_xlim(0,175); ax.set_ylim(0,42)
...
ax.set_xlabel('Time (h)'); ax.set_ylabel('Cumulative release (%)')
panel_label(ax,'(b) Korsmeyer\u2013Peppas fits'); legend_box(ax); ax.set_xlim(8,200); ax.set_ylim(0.5,60)
plt.tight_layout(pad=1.5)
...
aicc=[0,15.4,51.2,45.3,75.0]; colors=[RED,BLUE,ORANGE,PURPLE,GRAY]
...
ax.invert_yaxis(); ax.set_xlim(0,85); ax.grid(True,alpha=0.2,axis='x',zorder=0)
...
ax.axvline(x=s0,color=PURPLE,ls='--',lw=1.8,alpha=0.8,label=f'Fitted \u03c3 = {s0:.2f} min')
ax.fill_between([min(sigs),max(sigs)],[bt-1]*2,[bt+1]*2,alpha=0.08,color=ORANGE,ec='none')
...
oc={'4:1':RED,'1:1':BLUE,'1:4':ORANGE}; om={'4:1':'o','1:1':'s','1:4':'^'}; FOref=3.187
...
panel_label(ax,'(a) Concentration vs. flow rate'); legend_box(ax); ax.set_xlim(0.05,0.45)
...
panel_label(ax,'(b) Tracer flux vs. flow rate'); legend_box(ax); ax.set_xlim(0.05,0.45)
...
ax.plot([0,.35],[0,.35],'--',color=GRAY,lw=1,alpha=0.6,label='1:1')
r_val,_=sp_stats.pearsonr(aq,af)
ax.set_xlabel('Q$_{oil}$ (mL/min)'); ax.set_ylabel('F$_O$/F$_{O,ref}$')
panel_label(ax,'(c) Flux calibration'); legend_box(ax); ax.set_xlim(0,.35); ax.set_ylim(0,.35)
stat_box(ax,f'r = {r_val:.2f}\nRMSD = 8.3%')
plt.tight_layout(pad=1.5)
